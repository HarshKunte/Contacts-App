...
const AddContact = () => {
  ...
  const { currentUser, contactToUpdate, contactToUpdateKey } = state;
  ...
  const addContact = async () => {
    ...
    try {
      ...
      firebase.database()
        .ref('users/' + currentUser.uid + '/contacts/' + v4())
        .set({
          name, email, phoneNumber, address, picture: downloadUrl, star,
          createdAt: Date().split(' ').splice(0, 4).toString().replace(",", " ")
        })
  ...
  const updateContact = async () => {
    ...
    try {
      firebase.database()
        .ref('users/' + currentUser.uid + '/contacts/' + contactToUpdateKey)
        .set({
          name, email, phoneNumber, address, picture: downloadUrl, star,
          createdAt: Date().split(' ').splice(0, 4).toString().replace(",", " ")
        })
